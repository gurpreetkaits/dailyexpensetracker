<template>
  <div class="h-[calc(100vh-64px-56px)] md:h-auto md:lg:p-6 md:lg:ml-28 md:lg:mb-10">
    <div class="bg-white h-full md:rounded-lg md:shadow flex flex-col overflow-hidden md:min-w-full md:lg:min-h-[75vh]">
      ...
      <div class="bg-white px-4 py-3 border-t border-gray-100 flex items-center gap-2">
        <input v-model="inputMessage" @keyup.enter="handleSendMessage" :disabled="loading" type="text" placeholder="Type a message..." class="flex-1 rounded-full border border-gray-200 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#10b981] bg-gray-50" />
        <button @click="handleSendMessage" :disabled="loading" class="w-10 h-10 flex items-center justify-center rounded-full bg-[#10b981] hover:bg-green-600 text-white transition-colors">
    ...
    <!-- Subscription Modal -->
    <div v-if="showSubscriptionModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
      <div class="bg-white rounded-lg p-6 max-w-sm w-full mx-4">
        <div class="text-center">
          ...
          <div class="mb-6">
            ...
            <p class="text-3xl font-bold text-emerald-600 mt-2">$7<span class="text-lg font-normal text-gray-600">/month</span></p>
          </div>
          
          <div class="space-y-4 mb-6 text-left">
            <div class="flex items-start">
              <svg class="h-5 w-5 text-emerald-500 mt-0.5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
              </svg>
            ...
            <div class="flex items-start">
              ...
              <span class="text-gray-700">Multi Wallet <span class="text-xs text-emerald-600">(coming soon)</span></span>
            </div>
            <div class="flex items-start">
              <svg class="h-5 w-5 text-emerald-500 mt-0.5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
